...
function getE2EConfigFile(): IE2EDocsConfig | undefined {
	...
	try {
		...
		if (fs.existsSync(localDebugPath)) {
			...
			if (flagIndex > 0) {
				...
				console.log(`e2eConfigFile=${configPath}`);
				if (!fs.existsSync(configPath)) {
					console.log("Could not locate e2eDocsConfig.json used on the command.");
				}
	...
	if (config) {
		console.log(`Config=${JSON.stringify(config?.documents)}`);
	}
	return config;
}
